...
class RedisHealthChecker(BaseHealthChecker):
    ...
    def _get_client(self) -> redis.Redis:
        ...
        if self._use_pool:
            ...
            if pool is None:
                ...
                pool = connection_pool_manager.create_redis_pool(self._pool_key, pool_config)
    ...
    async def check_health(self) -> HealthCheckResult:
        ...
        try:
            ...
            self.logger.debug(f"Redis客户端已创建，连接到 {self.config.get('host')}:{self.config.get('port', 6379)}")
            ...
            self.logger.debug(f"PING命令执行完成，结果: {ping_result}, 耗时: {ping_time:.3f}秒")
            
            if ping_result:
                is_healthy = True
                metadata['ping_time'] = ping_time
                self.logger.info(f"Redis服务 {self.name} PING测试成功，响应时间: {ping_time:.3f}秒")
                ...
                if self.config.get('test_operations', False):
                    ...
                    if retrieved_value == test_value:
                        ...
                        self.logger.info(f"Redis服务 {self.name} SET/GET操作测试成功，SET耗时: {set_time:.3f}秒, GET耗时: {get_time:.3f}秒")
                    else:
                        is_healthy = False
                        error_message = "SET/GET操作测试失败"
                        metadata['operations_test'] = 'failed'
                        self.logger.error(f"Redis服务 {self.name} SET/GET操作测试失败，期望值: {test_value}, 实际值: {retrieved_value}")
                ...
                if self.config.get('collect_info', False):
                    try:
                        ...
                        self.logger.debug(f"Redis信息收集成功，版本: {info.get('redis_version')}, 连接数: {info.get('connected_clients')}")
        ...
        if is_healthy:
            self.logger.info(f"Redis服务 {self.name} 健康检查成功，总耗时: {response_time:.3f}秒")
        else:
            self.logger.warning(f"Redis服务 {self.name} 健康检查失败，总耗时: {response_time:.3f}秒，错误: {error_message}")
