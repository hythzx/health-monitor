...
class EMQXHealthChecker(BaseHealthChecker):
    ...
    async def _check_mqtt_connection(self) -> tuple[bool, float, Optional[str], Dict[str, Any]]:
        """
        通过MQTT协议检查EMQX连接
                    ...
                    try:
                        async with asyncio.timeout(2):  # 2秒超时
                            async for message in client.messages:
                                if message.topic.matches(test_topic) and message.payload.decode() == test_message:
                            ...
                            async with session.get(stats_url) as stats_response:
                                if stats_response.status == 200:
                                    ...
                                    if isinstance(stats_data, dict):
                                        metadata['connections_count'] = stats_data.get('connections.count', 0)
                                        metadata['sessions_count'] = stats_data.get('sessions.count', 0)
                                        metadata['topics_count'] = stats_data.get('topics.count', 0)
                                        metadata['subscriptions_count'] = stats_data.get('subscriptions.count', 0)
