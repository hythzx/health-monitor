...
class PerformanceMonitor:
    ...
    def _check_thresholds(self, metrics: PerformanceMetrics):
        ...
        if cpu_threshold and metrics.cpu_percent > cpu_threshold:
            self.logger.warning(f"CPU使用率超过阈值: {metrics.cpu_percent:.1f}% > {cpu_threshold}%")
            if self.on_threshold_exceeded:
                self.on_threshold_exceeded('cpu_percent', metrics.cpu_percent, cpu_threshold)
        
        # 检查内存使用率
        memory_threshold = self.alert_thresholds.get('memory_percent')
        if memory_threshold and metrics.memory_percent > memory_threshold:
            self.logger.warning(f"内存使用率超过阈值: {metrics.memory_percent:.1f}% > {memory_threshold}%")
            if self.on_threshold_exceeded:
                self.on_threshold_exceeded('memory_percent', metrics.memory_percent, memory_threshold)
