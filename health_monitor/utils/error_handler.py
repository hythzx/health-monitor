...
import logging
import time
from typing import Callable, Any, Optional, Dict, List, Union, TypeVar, Awaitable
from functools import wraps
from dataclasses import dataclass
from enum import Enum

from .exceptions import HealthMonitorError, ErrorCode
...
class ErrorHandler:
    ...
    def handle_error(
        ...
        if isinstance(error, HealthMonitorError):
            ...
            logger.error(f"处理系统错误: {error_msg}", extra={'error_details': error_dict})
        else:
                ...
                for attempt in range(1, config.max_attempts + 1):
                    try:
                        ...
                        if not retry_handler.should_retry(error, attempt):
                            logger.warning(f"错误不可重试或达到最大重试次数: {str(error)}")
                            raise
...
def setup_default_recovery_handlers():
    ...
    def handle_connection_error(error: Exception, context: Dict[str, Any]) -> Optional[Any]:
        """处理连接错误"""
    ...
    global_error_handler.register_recovery_handler(ConnectionError, handle_connection_error)
