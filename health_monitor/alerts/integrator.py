...
class AlertIntegrator:
    ...
    def _initialize_alerters(self, alert_configs: List[Dict[str, Any]]):
        ...
        for config in alert_configs:
            try:
                ...
                alerter_name = config.get('name', f'alerter_{len(self.alert_manager.alerters)}')
                ...
                self.logger.error(f"初始化告警器失败 {config.get('name', 'unknown')}: {e}")
    ...
    def create_service_filter(self, allowed_services: List[str]) -> Callable[[StateChange], bool]:
        """创建服务过滤器
    ...
    def create_status_filter(self, alert_on_down: bool = True, alert_on_up: bool = True) -> Callable[[StateChange], bool]:
        """创建状态过滤器
    ...
    def create_time_filter(self, quiet_hours: List[tuple]) -> Callable[[StateChange], bool]:
        """创建时间过滤器
