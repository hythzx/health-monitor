...
from ..utils.exceptions import CheckerError, ConfigError
...
class MonitorScheduler:
    ...
    def __init__(self, max_concurrent_checks: int = 10, enable_performance_monitoring: bool = True):
        """初始化监控调度器
        ...
        if enable_performance_monitoring:
            ...
            self.performance_monitor.set_threshold_callback(self._on_performance_threshold_exceeded)
        
        # 回调函数
        self.on_check_result: Optional[Callable[[HealthCheckResult], Awaitable[None]]] = None
        self.on_check_error: Optional[Callable[[str, Exception], Awaitable[None]]] = None
        self.on_performance_alert: Optional[Callable[[str, float, float], Awaitable[None]]] = None
    ...
    def configure_services(self, services_config: Dict[str, Any], 
        ...
        for service_name, service_config in services_config.items():
            try:
                # 创建检查器
                checker = health_checker_factory.create_checker(service_name, service_config)
                ...
                self.logger.info(f"配置服务 {service_name}: 类型={service_config.get('type')}, 间隔={check_interval}秒")
                
            except Exception as e:
                self.logger.error(f"配置服务 {service_name} 失败: {e}")
                raise
    
    def set_check_result_callback(self, callback: Callable[[HealthCheckResult], Awaitable[None]]):
        """设置检查结果回调函数
    ...
    def set_check_error_callback(self, callback: Callable[[str, Exception], Awaitable[None]]):
        """设置检查错误回调函数
    ...
    def set_performance_alert_callback(self, callback: Callable[[str, float, float], Awaitable[None]]):
        """设置性能告警回调函数
    ...
    def update_check_interval(self, service_name: str, interval: int):
        ...
        self.logger.info(f"更新服务 {service_name} 检查间隔: {old_interval}s -> {interval}s")
    ...
    def _on_performance_threshold_exceeded(self, metric_name: str, current_value: float, threshold: float):
        """性能阈值超限回调
        ...
        Args:
            ...
            threshold: 阈值
        """
        self.logger.warning(f"性能指标 {metric_name} 超过阈值: {current_value:.1f} > {threshold:.1f}")
        
        # 异步调用告警回调
        if self.on_performance_alert:
            asyncio.create_task(self.on_performance_alert(metric_name, current_value, threshold))
    ...
    def get_performance_metrics(self, minutes: int = 10) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            'history': [m.to_dict() for m in self.performance_monitor.get_metrics_history(minutes)]
        }
