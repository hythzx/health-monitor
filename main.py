...
class HealthMonitorApp:
    ...
    async def initialize(self):
        ...
        try:
            ...
            enable_performance_monitoring = config.get('global', {}).get('enable_performance_monitoring', True)
            self.monitor_scheduler = MonitorScheduler(max_concurrent, enable_performance_monitoring)
    ...
    def _configure_logging(self, global_config: Dict[str, Any]):
        ...
        if 'log_file' in global_config:
            ...
            log_config['max_file_size'] = global_config.get('max_log_size', 10 * 1024 * 1024)
    ...
    async def _handle_performance_alert(self, metric_name: str, current_value: float, threshold: float):
        """处理性能告警
        ...
        Args:
            ...
            threshold: 阈值
        """
        self.logger.warning(f"性能告警: {metric_name} = {current_value:.1f} (阈值: {threshold:.1f})")
        
        # 可以在这里发送性能告警通知
        # 例如通过告警系统发送性能相关的告警
    
    def _on_config_changed_callback(self, old_config: Dict[str, Any], new_config: Dict[str, Any]):
        """配置文件变更回调"""
        try:
    ...
    try:
        ...
        for service_name, result in results.items():
            if result is None:
                ...
                print(f"   ✅ {service_name}: 健康 (响应时间: {result.response_time:.3f}s)")
            else:
